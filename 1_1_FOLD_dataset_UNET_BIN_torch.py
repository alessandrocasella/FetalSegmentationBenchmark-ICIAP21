...
from torch.nn import functional as Fn, Parameter
...
class _BatchInstanceNorm(_BatchNorm):
    ...
    def forward(self, input):
        ...
        if self.affine:
            ...
            bn_w = self.gate
        out_bn = torch.nn.functional.batch_norm(
        ...
        # this did not forward in the following F.batch_norm(input,...) for 2D input tensors and 1d norm ..
        if input.dim() != 2:
            input = input.reshape(1, b * c, *input.size()[2:])

        out_in = torch.nn.functional.batch_norm(

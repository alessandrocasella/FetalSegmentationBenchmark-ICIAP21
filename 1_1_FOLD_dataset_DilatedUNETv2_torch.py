...
class PSPModule(nn.Module):
    ...
    def forward(self, feats):
        ...
        priors = [torch.nn.functional.upsample(input=stage(feats), size=(h, w), mode='bilinear') for stage in self.stages] + [feats]
...
class PSPUpsample(nn.Module):
    ...
    def forward(self, x):
        ...
        p = torch.nn.functional.upsample(input=x, size=(h, w), mode='bilinear')
        return self.conv(p)
...
class UNet(nn.Module):
    ...
    def __init__(self, n_class):
        ...
        self.psp = PSPModule(3, 128, (1,2,3,6))
        self.pspup = PSPUpsample(1024, 64)
    ...
    def forward(self, x):
        ...
        x = self.trans4(x)
        x = self.up_conv4(torch.cat([x, conv1, b1], dim=1))

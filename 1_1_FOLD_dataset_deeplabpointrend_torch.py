...
from prend.model import deeplabv3, PointHead, PointRend, point_sample
from prend.utils.gpus import reduce_tensor
...
result_model_path = os.path.join(Result_path, 'UNET_BIN')
...
class ComboLOSS(nn.Module):
    ...
    def forward(self, inputs, targets, smooth=1):
        ...
        Combo_loss = torch.nn.functional.cross_entropy(inputs, targets, ignore_index=255)
...
for k in range(0,1):
    ...
    model = PointRend(
        deeplabv3(pretrained=False, resnet="res101", head_in_ch=2048, num_classes=2),
        PointHead(in_c=514, num_classes=2, k=3, beta=0.75)
    ).to(device)
    #model = model.to(device, non_blocking=True)
    params = [{"params": model.backbone.backbone.parameters(), "lr": learning_rate},
              {"params": model.head.parameters(), "lr": learning_rate},
              {"params": model.backbone.classifier.parameters(), "lr": learning_rate * 10}]
    ...
    dataloader = DataLoader(cell_dataset, batch_size=batchSize, shuffle=True, collate_fn=collate_fn, num_workers=4,
                            pin_memory=True)
    ...
    for epoch in range(epochs):
        ...
        for iteration, (input_train, target_train) in enumerate(tqdm(dataloader)):
            ...
            target_train = target_train.to(device, dtype=torch.long, non_blocking=True).squeeze_(1)


            output = model(input_train)
            pred = torch.nn.functional.interpolate(output["coarse"], input_train.shape[-2:], mode="bilinear", align_corners=True)
            seg_loss = torch.nn.functional.cross_entropy(pred, target_train, ignore_index=255)

            gt_points = point_sample(
                target_train.float().unsqueeze(1),
                output["points"],
                mode="nearest",
                align_corners=False
            ).squeeze_(1).long()
            points_loss = torch.nn.functional.cross_entropy(output["rend"], gt_points, ignore_index=255)

            #loss = seg_loss + points_loss

            tdice = reduce_tensor(seg_loss) # segloss
            tssim = reduce_tensor(points_loss) #pointloss
            loss = tdice + tssim
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        for iteration, (input_val, target_val) in enumerate(val_dataloader):
            ...
            with torch.no_grad():
                scores = model(input_val)["fine"].argmax(1).float().unsqueeze(1)
                test = scores
                test = test.to('cpu')
        ...
        if np.mean(avg_loss) < best_loss:
            ...
            print('Saving model')
            torch.save(model, os.path.join(os.getcwd(), 'RESULTS/UNET_BIN/K FOLD- 2 - NI - REDUCED FOV - POLIMI DATASET/model_unet_checkpoint_{:02d}_fold.pth'.format(k+1)))
...
for k in range(0, 1):
    model = PointRend(
        deeplabv3(pretrained=False, resnet="res101", head_in_ch=2048, num_classes=2),
        PointHead(in_c=513, num_classes=2, k=3, beta=0.75)
    ).to(device)
    print('Fold{}'.format(k+1))
    #path_model = K_path_model[k]
    model = torch.load(os.path.join(os.getcwd(), 'RESULTS/UNET_BIN/K FOLD- 2 - NI - REDUCED FOV - POLIMI DATASET/model_unet_checkpoint_{:02d}_fold.pth'.format(k+1)))
    model.eval()
    test_image = []
    predicted_4d = []
    predicted_3d = []
    ...
    for image in test_image_list:
        ...
        with torch.no_grad():
            prediction = model(im)["fine"].argmax(1).float().unsqueeze(1)

...
K_FOLD = os.path.join(os.getcwd(), 'K FOLD - 2 - BLUR - CONTRAST - NI - REDUCED FOV - POLIMI DATASET')
print(K_FOLD)
...
Result_path = os.path.join(os.getcwd(),'RESULT_DENSEUNET')
result_model_path = os.path.join(Result_path, 'Model 3')
result_dataset_path = os.path.join(result_model_path, 'BLUR - CONTRAST - NI - REDUCED FOV')
try:
...
def dense_layer(input, filter):
    # print('dense_layer')
    d = BatchNormalization()(input)
    d = Dropout(0.1)(d)
    d = Conv2D(filter, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same')(d)
    d = BatchNormalization()(d)
    return d


def transition_down(input, filter):
    # print('transition_down')
    t = BatchNormalization()(input)
    t = Dropout(0.1)(t)
    t = Conv2D(filter, (1, 1), activation='relu', kernel_initializer='he_normal', padding='same')(t)
    t = BatchNormalization()(t)
    p = MaxPooling2D((2, 2))(t)
    return p
...
class myUnet(object):
    ...
    def get_unet(self, training=True):
        inputs = Input((self.img_rows, self.img_cols, 3))  # input

        c1 = Conv2D(32, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same')(
            inputs)  # first convolution layer

        # dense block: 2 - dense layer
        d1 = dense_layer(c1, 32)
        d1 = dense_layer(d1, 32)

        # concatenate before Transition down: concatation between first convolutional layer output and dense block output
        cc1 = concatenate([c1, d1])

        # Transition down:
        t1 = transition_down(cc1, 64)

        # dense block: 3 - dense layer
        d2 = dense_layer(t1, 64)

        d2 = dense_layer(d2, 64)

        d2 = dense_layer(d2, 64)

        # concatenate before Transition down: concatation between first convolutional layer output and dense block output
        cc2 = concatenate([t1, d2])

        # Transition down:
        t2 = transition_down(cc2, 128)

        # dense block: 4 - dense layer
        d3 = dense_layer(t2, 128)
        d3 = dense_layer(d3, 128)
        d3 = dense_layer(d3, 128)
        d3 = dense_layer(d3, 128)

        # concatenate before Transition down: concatation between first convolutional layer output and dense block output
        cc3 = concatenate([t2, d3])

        # Transition down:
        t3 = transition_down(cc3, 256)

        # dense block: 5 - dense layer
        d4 = dense_layer(t3, 256)
        d4 = dense_layer(d4, 256)
        d4 = dense_layer(d4, 256)
        d4 = dense_layer(d4, 256)
        d4 = dense_layer(d4, 256)

        # concatenate before Transition down: concatation between first convolutional layer output and dense block output
        cc4 = concatenate([t3, d4])

        # Transition down:
        t4 = transition_down(cc4, 512)

        # dense block:  6 - dense layer
        d5 = dense_layer(t4, 512)
        d5 = dense_layer(d5, 512)
        d5 = dense_layer(d5, 512)
        d5 = dense_layer(d5, 512)
        d5 = dense_layer(d5, 512)
        d5 = dense_layer(d5, 512)

        # Transition up: concatation between first convolutional layer output and dense block output
        u6 = Conv2DTranspose(128, (3, 3), strides=(2, 2), padding='same')(d5)
        u6 = concatenate([u6, cc4])

        # dense block:  5 - dense layer
        c6 = dense_layer(u6, 256)
        c6 = dense_layer(c6, 256)
        c6 = dense_layer(c6, 256)
        c6 = dense_layer(c6, 256)
        c6 = dense_layer(c6, 256)
        ...
        # dense block:  4 - dense layer
        c7 = dense_layer(u7, 128)
        c7 = dense_layer(c7, 128)
        c7 = dense_layer(c7, 128)
        c7 = dense_layer(c7, 128)
        ...
        # dense block:  3 - dense layer
        c8 = dense_layer(u8, 64)
        c8 = dense_layer(c8, 64)
        c8 = dense_layer(c8, 64)
        ...
        # dense block:  2 - dense layer
        c9 = dense_layer(u9, 32)
        c9 = dense_layer(c9, 32)
...
class DataGenerator(tf.keras.utils.Sequence):
    ...
    def __data_generation(self, file_list_temp):
        ...
        y_new = np.empty([self.batch_size, 128, 128, 1], dtype=np.float32)
        x_new = np.empty([self.batch_size, 128, 128, 3], dtype=np.float32)
        for p, ID in enumerate(file_list_temp):
            x_file_path = os.path.join(self.dataDir, ID)
            y_file_path = os.path.join(self.labelDir, ID)

            transformInstance = self.aug.get_random_transform((128, 128, 3))
            # X = np.array(np.load(x_file_path)).astype('float32')
            X = Image.fromarray(np.array(np.load(x_file_path)))
            X = X.resize((128,128), Image.ANTIALIAS)
            X = np.array(np.array(X) / 255.).astype('float32')

            Z = X
            # y = np.array(np.load(y_file_path)).astype('float32')
            y_old = np.array(np.load(y_file_path))
            y_old = Image.fromarray(np.dstack((y_old, y_old, y_old)))
            y_old = y_old.resize((128,128))
...
for k in range(0,1):
    ...
    checkpointer = ModelCheckpoint(os.path.join(os.getcwd(), 'RESULT_DENSEUNET/Model 3/BLUR - CONTRAST - NI - REDUCED FOV/model_dense_unet_checkpoint_{:02d}_fold.h5'.format(k+1)), verbose=1, save_best_only=True, monitor='val_loss', mode = 'min')
    ...
    K_path_model.append(os.path.join(os.getcwd(),'RESULT_DENSEUNET/Model 3/BLUR - CONTRAST - NI - REDUCED FOV/model_dense_unet_checkpoint_{:02d}_fold.h5'.format(k + 1)))
    # saving the metrics' value in a dataset
    with h5py.File(os.path.join(os.getcwd(), 'RESULT_DENSEUNET/Model 3/BLUR - CONTRAST - NI - REDUCED FOV/FOLD{0}_Metrics_history_dense_unet.hdf5'.format(k + 1)), 'w') as f:
        ...
        f.create_dataset('loss', data=loss_history)
        f.create_dataset('val_loss', data=val_loss_history)
        f.close()
...
for k in range(0, 1):
    ...
    myunet = load_model(os.path.join(os.getcwd(), 'RESULT_DENSEUNET/Model 3/BLUR - CONTRAST - NI - REDUCED FOV/model_dense_unet_checkpoint_{:02d}_fold.h5'.format(k+1)), compile=False)
    test_image = []
    predicted_4d = []
    predicted_3d = []
    pred_thr = []
    ...
    for image in test_image_list:
        ...
        im = np.load(t_file_path) # trasforming the image in float 32 with np.max 0< 1

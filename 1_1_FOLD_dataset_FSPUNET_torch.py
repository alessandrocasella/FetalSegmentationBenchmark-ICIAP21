...
class _ConvBnReLU(nn.Sequential):
    ...
    def __init__(
        ...
        self.add_module("bn", torch.nn.BatchNorm2d(out_ch, eps=1e-5, momentum=1 - 0.999))
...
class _ImagePool(nn.Module):
    ...
    def forward(self, x):
        ...
        h = self.pool(x)
        h = self.conv(h)
        h = torch.nn.functional.interpolate(h, size=(H, W), mode="bilinear", align_corners=False)
        return h

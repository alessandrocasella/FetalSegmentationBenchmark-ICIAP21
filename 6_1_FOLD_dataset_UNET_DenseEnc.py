...
result_model_path = os.path.join(Result_path, 'ATTENTIONUNET')
...
for k in range(0,k_fold):
    ...
    checkpointer = ModelCheckpoint(os.path.join(os.getcwd(), 'RESULTS/ATTENTIONUNET/K FOLD- 2 - NI - REDUCED FOV - POLIMI DATASET/model_unet_attention_checkpoint_{:02d}_fold.h5'.format(k+1)), verbose=1, save_best_only=True, monitor='val_loss', mode = 'min')
...
for k in range(0, k_fold):
    ...
    myunet = load_model(os.path.join(os.getcwd(), 'RESULTS/ATTENTIONUNET/K FOLD- 2 - NI - REDUCED FOV - POLIMI DATASET/model_unet_attention_checkpoint_{:02d}_fold.h5'.format(k+1)), compile=False)
    test_image = []
    predicted_4d = []
    predicted_3d = []
    pred_thr = []

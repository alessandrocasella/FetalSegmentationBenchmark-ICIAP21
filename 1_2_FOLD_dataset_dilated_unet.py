''' To restore the training for all the folds delete or comment the line 34'''
...
result_model_path = os.path.join(Result_path, 'Model 4')
...
class myUnet(object):
    ...
    def get_unet(self, training=True):
        ...
        c1 = Conv2D(32, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same', dilation_rate=1)(
            inputs)
        c1 = BatchNormalization()(c1)
        c1 = Dropout(0.1)(c1)
        c1 = Conv2D(32, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same', dilation_rate=1)(c1)
        c1 = BatchNormalization()(c1)
        p1 = AveragePooling2D((2, 2))(c1)

        c2 = Conv2D(64, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same', dilation_rate=2)(p1)
        c2 = BatchNormalization()(c2)
        c2 = Dropout(0.1)(c2)
        c2 = Conv2D(64, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same', dilation_rate=2)(c2)
        c2 = BatchNormalization()(c2)
        p2 = AveragePooling2D((2, 2))(c2)

        c3 = Conv2D(128, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same', dilation_rate=4)(p2)
        c3 = BatchNormalization()(c3)
        c3 = Dropout(0.2)(c3)
        c3 = Conv2D(128, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same', dilation_rate=4)(c3)
        c3 = BatchNormalization()(c3)
        p3 = AveragePooling2D((2, 2))(c3)

        d1 = Conv2D(256, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same', dilation_rate=1)(p3)
        d1 = BatchNormalization()(d1)

        d2 = Conv2D(256, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same', dilation_rate=2)(p3)
        d2 = BatchNormalization()(d2)

        d3 = Conv2D(256, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same', dilation_rate=4)(p3)
        d3 = BatchNormalization()(d3)

        d4 = Conv2D(256, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same', dilation_rate=8)(p3)
        d4 = BatchNormalization()(d4)

        d5 = Conv2D(256, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same', dilation_rate=16)(
            p3)
        d5 = BatchNormalization()(d5)

        d6 = Conv2D(256, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same', dilation_rate=32)(
            p3)
        d6 = BatchNormalization()(d6)

        dilate_all_added = add([d1, d2, d3, d4, d5, d6])
        c = Conv2D(256, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same')(dilate_all_added)

        u7 = UpSampling2D((2, 2), interpolation='nearest')(c)
...
for k in range(5,k_fold):
    ...
    checkpointer = ModelCheckpoint(os.path.join(os.getcwd(), 'RESULT/Model 4/NI - REDUCED FOV/model_unet_dilated_checkpoint_{:02d}_fold.h5'.format(k+1)), verbose=1, save_best_only=True, monitor='val_loss', mode = 'min')
    ...
    K_path_model.append(os.path.join(os.getcwd(),'RESULT/Model 4/NI - REDUCED FOV/model_unet_dilated_checkpoint_{:02d}_fold.h5'.format(k + 1)))
    # saving the metrics' value in a dataset
    with h5py.File(os.path.join(os.getcwd(), 'RESULT/Model 4/NI - REDUCED FOV/FOLD{0}_Metrics_history_dilated.hdf5'.format(k + 1)), 'w') as f:
...
for k in range(0, k_fold):
    ...
    myunet = load_model(os.path.join(os.getcwd(), 'RESULT/Model 4/NI - REDUCED FOV/model_unet_dilated_checkpoint_{:02d}_fold.h5'.format(k+1)), compile=False)
    test_image = []
    predicted_4d = []
    predicted_3d = []
    pred_thr = []

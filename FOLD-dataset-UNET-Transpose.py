...
import h5py
# test mixed precision per dare più velocità
#from tensorflow.keras.mixed_precision import experimental as mixed_precision
#policy = mixed_precision.Policy('mixed_float16')
#mixed_precision.set_policy(policy)
#

K_FOLD = os.path.join(os.getcwd(), 'K FOLD - NI - REDUCED FOV - POLIMI DATASET')
print(K_FOLD)
...
if (len(test_path_image_list) != 12):
    print('Error in image test path')
if (len(train_path_image_list) != 12):
    print('Error in image train path')
if (len(val_path_image_list) != 12):
    print('Error in image val path')

if (len(test_path_mask_list) != 12):
    print('Error in mask test path')
if (len(train_path_mask_list) != 12):
    print('Error in mask train path')
if (len(val_path_mask_list) != 12):
...
result_model_path = os.path.join(Result_path, 'Model 1')
...
for k in range(0,k_fold):
    ...
    checkpointer = ModelCheckpoint(os.path.join(os.getcwd(), 'RESULT/Model 1/NI - REDUCED FOV/model_unet_transpose_checkpoint_{:02d}_fold.h5'.format(k+1)), verbose=1, save_best_only=True, monitor='val_loss', mode = 'min')
    ...
    K_path_model.append(os.path.join(os.getcwd(),'RESULT/Model 1/NI - REDUCED FOV/model_unet_transpose_checkpoint_{:02d}_fold.h5'.format(k + 1)))
    # saving the metrics' value in a dataset
    with h5py.File(os.path.join(os.getcwd(), 'RESULT/Model 1/NI - REDUCED FOV/FOLD{0}_Metrics_history_transpose.hdf5'.format(k + 1)), 'w') as f:
...
for k in range(0, k_fold):
    ...
    myunet = load_model(path_model, compile=False)
    test_image = []
    predicted_4d = []
    predicted_3d = []
    pred_thr = []
...
for k in range(0, k_fold):
    ...
    while (l < len(test_image)):
        fig, axs = plt.subplots(3, 4, gridspec_kw={'hspace': 0.1, 'wspace': 0.1}, figsize=(10, 10))
        fig.suptitle('FOLD {0} - Testing: number {1}-{2}-{3}'.format(k+1,l, l+1, l+2), fontsize=20)
        axs = axs.ravel()

...
result_model_path = os.path.join(Result_path, 'RESUNET_IN')
...
class PaddedConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        * in : torch.Tensor
        ...
        #pad_h = np.int(np.ceil((self.conv2d.kernel_size[0] - x.shape[2] * (1 - self.conv2d.stride[0]) - self.conv2d.stride[0]) / 2))
        #pad_w = np.int(np.ceil((self.conv2d.kernel_size[1] - x.shape[3] * (1 - self.conv2d.stride[1]) - self.conv2d.stride[1]) / 2))
        pad_h = 0
        pad_w = 0
        padder = self.pad2d_type((pad_w, pad_w, pad_h, pad_h))

        return self.conv2d(padder(x))


class ConvBlock(nn.Module):
    def __init__(self, input_size, sizes):
        super(ConvBlock, self).__init__()
        self.conv1 = PaddedConv2d(nn.Conv2d(input_size, sizes[0], 1), nn.ZeroPad2d)
        self.bn1 = nn.InstanceNorm2d(sizes[1], affine=True)
        self.conv2 = PaddedConv2d(nn.Conv2d(sizes[0], sizes[1], 3, padding=1), nn.ZeroPad2d)
        self.bn2 = nn.InstanceNorm2d(sizes[1], affine=True)
        self.conv3 = PaddedConv2d(nn.Conv2d(sizes[1], sizes[2], 1), nn.ZeroPad2d)
        self.bn3 = nn.InstanceNorm2d(sizes[2], affine=True)
        self.activation = nn.LeakyReLU()
        self.shortcut = PaddedConv2d(nn.Conv2d(input_size, sizes[2], 1), nn.ZeroPad2d)
...
class IdentityBlock(nn.Module):
    def __init__(self, input_size, sizes):
        ...
        self.conv1 = PaddedConv2d(nn.Conv2d(input_size, sizes[0], 1), nn.ZeroPad2d)
        self.bn1 = nn.InstanceNorm2d(sizes[0], affine=True)
        self.conv2 = PaddedConv2d(nn.Conv2d(sizes[0], sizes[1], 3, padding=1), nn.ZeroPad2d)
        self.bn2 = nn.InstanceNorm2d(sizes[1], affine=True)
        self.conv3 = PaddedConv2d(nn.Conv2d(sizes[1], sizes[2], 1), nn.ZeroPad2d)
        self.bn3 = nn.InstanceNorm2d(sizes[2], affine=True)
        self.activation = nn.LeakyReLU()
        #self.shortcut = PaddedConv2d(nn.Conv2d(sizes[0], sizes[2], 1), nn.ZeroPad2d)
...
class UNet(nn.Module):
    def __init__(self, out_classes=1):
        ...
        self.down_conv2 = DownBlock(64, 256)
        self.down_conv3 = DownBlock(128, 512)
        self.down_conv4 = DownBlock(256, 1024)

        self.double_conv = DoubleConv(512, 2048)

        self.up_conv4 = UpBlock(2048, 1024)
        self.up_conv3 = UpBlock(1024, 512)
        self.up_conv2 = UpBlock(512, 256)
        self.up_conv1 = UpBlock(256, 128)

        self.conv_last = nn.Conv2d(128, out_classes, kernel_size=1)
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.trans1(out)
        out = self.up_conv1(torch.cat([out, s4], dim=1))

        out = self.trans2(out)
        out = self.up_conv2(torch.cat([out, s3], dim=1))

        out = self.trans3(out)
        out = self.up_conv3(torch.cat([out, s2], dim=1))

        out = self.trans4(out)
        out = self.up_conv4(torch.cat([out, s1], dim=1))
    ...
    for epoch in range(epochs):
        ...
        if np.mean(avg_loss) < best_loss:
            ...
            torch.save(model, os.path.join(os.getcwd(),
                                           'RESULTS/RESUNET_IN/K FOLD- 2 - NI - REDUCED FOV - POLIMI DATASET/model_unet_checkpoint_{:02d}_fold.pth'.format(
                                               k + 1)))

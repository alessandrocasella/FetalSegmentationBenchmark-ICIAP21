...
for k in range(0,k_fold):
    if (k == 5):
        ...
        checkpointer = ModelCheckpoint(os.path.join(os.getcwd(), 'RESULT_2/Model 1/NI - REDUCED FOV/model_unet_transpose_checkpoint_{:02d}_fold.h5'.format(k+1)), verbose=1, save_best_only=True, monitor='val_loss', mode = 'min')
        ...
        K_path_model.append(os.path.join(os.getcwd(),'RESULT_2/Model 1/NI - REDUCED FOV/model_unet_transpose_checkpoint_{:02d}_fold.h5'.format(k + 1)))
        # saving the metrics' value in a dataset
        with h5py.File(os.path.join(os.getcwd(), 'RESULT_2/Model 1/NI - REDUCED FOV/FOLD{0}_Metrics_history_transpose.hdf5'.format(k + 1)), 'w') as f:
...
for k in range(0, k_fold):
    ...
    myunet = load_model(os.path.join(os.getcwd(),'RESULT_2/Model 1/NI - REDUCED FOV/model_unet_transpose_checkpoint_{:02d}_fold.h5'.format(k+1)), compile=False)
    test_image = []
    predicted_4d = []
    predicted_3d = []
    pred_thr = []

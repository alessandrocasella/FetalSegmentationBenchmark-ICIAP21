...
result_model_path = os.path.join(Result_path, 'ATTUNET_IN')
...
for k in range(5,k_fold):
    ...
    for epoch in range(epochs):
        ...
        if np.mean(avg_loss) < best_loss:
            ...
            print('Saving model')
            torch.save(model, os.path.join(os.getcwd(), 'RESULTS/ATTUNET_IN/K FOLD- 2 - NI - REDUCED FOV - POLIMI DATASET/model_unet_attention_checkpoint_{:02d}_fold.pth'.format(k+1)))
    ...
    K_path_model.append(os.path.join(os.getcwd(),'RESULTS/ATTUNET_IN/K FOLD- 2 - NI - REDUCED FOV - POLIMI DATASET/model_unet_attention_checkpoint_{:02d}_fold.h5'.format(k + 1)))
    # saving the metrics' value in a dataset
    with h5py.File(os.path.join(os.getcwd(), 'RESULTS/ATTUNET_IN/K FOLD- 2 - NI - REDUCED FOV - POLIMI DATASET/FOLD{0}_Metrics_history_upsampling.hdf5'.format(k + 1)), 'w') as f:
...
for k in range(0, k_fold):
    ...
    model = torch.load(os.path.join(os.getcwd(), 'RESULTS/ATTUNET_IN/K FOLD- 2 - NI - REDUCED FOV - POLIMI DATASET/model_unet_attention_checkpoint_{:02d}_fold.pth'.format(k+1)))
    model.eval()
    test_image = []
    predicted_4d = []
    predicted_3d = []

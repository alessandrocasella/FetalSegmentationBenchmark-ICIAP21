...
class SkipConv(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.skipconv = nn.Conv2d(
            in_channels=in_channels, out_channels=out_channels,
            kernel_size=1, stride=1, padding=0, bias=True
        )

    def forward(self, x):
        return self.skipconv(x)
...
class UpSampler(nn.Sequential):
    def __init__(self, in_channels, out_channels, scale):
        ...
        self.convTrans = nn.ConvTranspose2d(
            in_channels=in_channels, out_channels=out_channels,
            kernel_size=3, stride=scale, padding=0, output_padding=scale, bias=True)
...
class FCDenseNet(nn.Module):
    def __init__(self, in_channels=3, down_blocks=(5,5,5,5,5),
        ...
        for i in range(len(down_blocks)):
            ...
            self.skipconv.append(SkipConv(cur_channels_count, cur_channels_count))
        ...
        self.upconvUp = nn.ModuleList([])
        self.denseBlocksUp = nn.ModuleList([])
        for i in range(len(up_blocks)-1):
            self.transUpBlocks.append(TransitionUp(prev_block_channels, prev_block_channels))
            cur_channels_count = prev_block_channels + skip_connection_channel_counts[i]
            self.upconvUp.append(UpSampler(cur_channels_count,cur_channels_count,((len(up_blocks))*2-i*2)))
            print((len(up_blocks))*2-i*2)
...
for k in range(0,k_fold):
    ...
    for epoch in range(epochs):
        ...
        if (np.mean(avg_loss) < best_loss) and (np.mean(avg_loss_train) <= np.mean(avg_loss)):
            ...
            print('Saving model')
            torch.save(model, os.path.join(os.getcwd(), 'RESULTS/DENSE_IN_FSP/K FOLD- 2 - NI - REDUCED FOV - POLIMI DATASET/model_unet_checkpoint_{:02d}_fold.pth'.format(k+1)))
...
for k in range(0, k_fold):
    ...
    model = torch.load(os.path.join(os.getcwd(), 'RESULTS/DENSE_IN_FSP/K FOLD- 2 - NI - REDUCED FOV - POLIMI DATASET/model_unet_checkpoint_{:02d}_fold.pth'.format(k+1)))
    model.eval()
    test_image = []
    predicted_4d = []
    predicted_3d = []

...
directory = 'BLUR - CONTRAST -  NI -  REDUCED FOV - POLIMI DATASET'
...
for l, path in enumerate(type_path_list):
    image_path = os.path.join(path, 'Image_Blur')
...
for x, path in enumerate(path_image_list_new):
    ...
    for image in image_list_pil:
        ...
        image_cv = cv2.imread(im)  # imread vuole il path
        blur = cv2.GaussianBlur(image_cv, (, 5), 0)
        cv2_imshow(blur)
        ...
        # print(image_cv)
        lab = cv2.cvtColor(np.array(blur), cv2.COLOR_BGR2LAB)
        # print('prima')
        ...
        if (final.shape != (256, 256, 3)):
            print('error shape')
        image_list_blur.append(final)


    if (len(image_list_blur) != 100):
        print('error len')
    patient_blur_image_list.append(image_list_blur)

    # saving in PNG
    for l, image in enumerate(image_list_blur):
        cv2.imwrite(os.path.join(patient_image_path_list_png[x], 'image_blur_{:03d}.png'.format(l)), image)
...
for patient, image_list in enumerate(patient_blur_image_list):
    image_np = []
    for image in image_list:
        im = np.asarray(image)

...
Result_path = os.path.join(os.getcwd(),'RESULT_2')
result_model_path = os.path.join(Result_path, 'Model 1 - PATCH')
...
class DataGenerator(tf.keras.utils.Sequence):
    ...
    def __data_generation(self, file_list_temp):
        ...
        y_new = np.empty([self.batch_size * 4, 128, 128, 1], dtype=np.float32)
        x_new = np.empty([self.batch_size * 4, 128, 128, 3], dtype=np.float32)
        # print(len(file_list_temp))
        p = 0
        for h, ID in enumerate(file_list_temp):
            ...
            X_tiles = [X[x:x + M, y:y + N] for x in range(0, X.shape[0], M) for y in range(0, X.shape[1], N)]
            ...
            M = Y.shape[0] // 2
            N = Y.shape[1] // 2
            Y_tiles = [Y[x:x + M, y:y + N] for x in range(0, Y.shape[0], M) for y in range(0, Y.shape[1], N)]
            for i, image in enumerate(X_tiles):
                Z = image
                y = Y_tiles[i]
                y = np.asarray(np.dstack((y, y, y)), dtype=np.float32)

                Z[:, :, :] = self.aug.apply_transform(image[:, :, :], transformInstance)
                '''if (np.max(Z)!= 1.0):
...
for k in range(2,k_fold):
        ...
        checkpointer = ModelCheckpoint(os.path.join(os.getcwd(), 'RESULT_2/Model 1 - PATCH/NI - REDUCED FOV/model_unet_transpose_patch_checkpoint_{:02d}_fold.h5'.format(k+1)), verbose=1, save_best_only=True, monitor='val_loss', mode = 'min')
        ...
        K_path_model.append(os.path.join(os.getcwd(),'RESULT_2/Model 1 - PATCH/NI - REDUCED FOV/model_unet_transpose_patch_checkpoint_{:02d}_fold.h5'.format(k + 1)))
        # saving the metrics' value in a dataset
        with h5py.File(os.path.join(os.getcwd(), 'RESULT_2/Model 1 - PATCH/NI - REDUCED FOV/FOLD{0}_Metrics_history_transpose_patch.hdf5'.format(k + 1)), 'w') as f:
...
for k in range(0, k_fold):
    ...
    myunet = load_model(os.path.join(os.getcwd(),'RESULT_2/Model 1 - PATCH/NI - REDUCED FOV/model_unet_transpose_patch_checkpoint_{:02d}_fold.h5'.format(k+1)), compile=False)
    test_image = []
    predicted_4d = []
    predicted_3d = []
    pred_thr = []
    ...
    for image in test_image_list:
        ...
        for value in tiles:
            prediction = myunet.predict(np.expand_dims(value, 0))
    ...
    i = 0
    prediction_3d_recomposed = []
    while (i < len(predicted_3d)):
        X_orig = np.zeros([256, 256, 1])
        X_orig[:128, :128] = predicted_3d[i]
        X_orig[:128, 128:256] = predicted_3d[i + 1]
        X_orig[128:256, :128] = predicted_3d[i + 2]
        X_orig[128:256, 128:256] = predicted_3d[i + 3]
        i = i + 4
        prediction_3d_recomposed.append(X_orig)
    i = 0
    pred_thr_recomposed = []
    while (i < len(pred_thr)):
        X_orig = np.zeros([256, 256, 1])
        X_orig[:128, :128] = pred_thr[i]
        X_orig[:128, 128:256] = pred_thr[i + 1]
        X_orig[128:256, :128] = pred_thr[i + 2]
        X_orig[128:256, 128:256] = pred_thr[i + 3]
        i = i + 4
        pred_thr_recomposed.append(X_orig)
    print('The Image tested are {1} and the prediction are {0}'.format(len(predicted_4d), len(test_image)))
    print('The reshaped prediction are {0}'.format(len(prediction_3d_recomposed)))
    print('The thresholded prediction are {0}'.format(len(pred_thr_recomposed)))
    K_test_thr_predicted.append(pred_thr_recomposed)
    K_test_predicted.append(prediction_3d_recomposed)

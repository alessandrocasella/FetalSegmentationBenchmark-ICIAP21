...
class UpSampler(nn.Module):
    ...
    def __init__(self, upsample_channels: int, scale_facotr: int):
        r"""
        ...
        self.out_channels = upsample_channels
...
class CenterCrop(nn.Module):
    ...
    def forward(self, x):
        res = self.center_crop(x, self.height, self.width)
        return res
...
class FCDenseNet(nn.Module):
    ...
    def __init__(self,
        ...
        for i, (transition_params_up, dense_params_up) in enumerate(up_pairs_params):
            ...
            scaler_out = UpSampler(current_channels, len(down_dense_num_layers)-1-i)
            current_channels = transition.out_channels
            self.up_trans.add_module(f'trans_{i}', transition)
            self.up_out.add_module(f'upscaler_{i}', scaler_out)
